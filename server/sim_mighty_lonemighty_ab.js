'use strict';
/**
 * A-B harness: 주공의 그 무늬가 마이티 한 장뿐일 때 그 무늬를 안 돌리는 좌석(hold)
 * vs 예전 좌석(legacy).
 * 한 테이블에서 같은 시드 딜을 두 번(캠프를 뒤집어) 쳐서 자리 편향을 씻는다.
 * 두 캠프 모두 mixoracle 이고 차이는 좌석 게이트(__mightyLoneMightySuitLegacy) 하나뿐이다.
 *
 ...
 * sim_mighty_top_dump.js 로 따로 본다.
 */
...
for (let pass = 0; pass < rounds * 2; pass++) {
  ...
  global.__mightyLoneMightySuitLegacy = (pid) => (!nullTest && seatCamp[pid] === 'legacy');
...
delete global.__mightyLoneMightySuitLegacy;

const pct = (a, b) => (b > 0 ? `${(a / b * 100).toFixed(1)}%` : '-');
console.log(`\n마이티 무늬 회피 A/B — ${rounds}딜 ×2(미러) (base ${base})${nullTest ? ' · NULL(A/A) 검증' : ''}`);
