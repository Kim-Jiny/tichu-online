...
async function main() {
  ...
  try {
    ...
    // ── 보상 없이 넘어가는 시즌 ────────────────────────────────────────
    // 티어를 다 비우는 건 "이번 시즌은 보상 없이 간다"는 운영 판단일 수 있다.
    // 시스템이 그걸 거부하면 시즌을 닫을 방법이 없어진다.
    await clearPayouts();
    ...
    check(noTiers.success, '티어가 비어 있어도 시즌은 닫힌다');
    check((await raw.query(
      `SELECT status FROM tc_seasons WHERE id = $1`, [s7])).rows[0].status === 'closed',
      '상태가 closed 로 바뀐다');
    check(await goldOf('일등') === 0, '대신 아무 골드도 나가지 않는다');
    check((await raw.query(
      `SELECT COUNT(*)::int n FROM tc_season_rewards WHERE season_id = $1`, [s7])).rows[0].n === 0,
      '지급 기록도 남지 않는다');

    // 그리고 그 선택은 재부팅에도 살아남아야 한다 — 예전에는 기본값이 비면
    // 부팅 때 자동으로 다시 채워져서, 안 주기로 한 결정이 조용히 뒤집혔다.
    await db.initDatabase();
    check((await raw.query(
      `SELECT COUNT(*)::int n FROM tc_season_reward_config WHERE season_id IS NULL`)).rows[0].n === 0,
      '비워둔 기본 보상은 재부팅해도 되살아나지 않는다');
