...
class L10nKo extends L10n {
  ...
  String get rulesMtScoringBody =>
      '덱에 점수 카드 20장 (A, K, Q, J, 10 × 4문양 = 20장).\n\n[기본 점수]\n• 성공 시: 기본 = (비딩 − 최소비딩 + 1) × 2 + (먹은 점수 − 비딩)\n• 실패 시: 기본 = (비딩 − 최소비딩 + 1) × 1 + (비딩 − 먹은 점수)   ← 크게 미달할수록 손실 큼\n(실패 시엔 기본 부분에만 ×2가 빠지고, 아래 배율은 그대로 적용)\n\n[점수 배분]\n• 주공: 기본 × 2\n• 프렌드: 기본 × 1\n• 야당 각: −기본\n실패 시 부호 반전.\n\n[배율 (기본 점수에 곱셈, 성공·실패 공통)]\n• 솔로 (노프렌드): ×2\n• 런 (20점 올킬): ×2\n• 노기루다 (NT): ×2\n• 20비딩: ×2\n최대 배율: ×16 (솔로 + 런 + NT + 20비딩)\n\n[성공 예시]\n13비딩, 프렌드 있음, 15점 획득:\n기본 = (1×2) + 2 = 4\n주공 +8, 프렌드 +4, 야당 각 −4\n\n[실패 예시]\n14비딩, 프렌드 있음, 5점 획득 (9점 미달):\n기본 = (2×1) + 9 = 11\n주공 −22, 프렌드 −11, 야당 각 +11';
