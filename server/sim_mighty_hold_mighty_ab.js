'use strict';
/**
 * A-B harness: 마이티를 아끼는 좌석(hold) vs 예전처럼 마이티를 쓰는 좌석(legacy).
 * 한 테이블에서 같은 시드 딜을 두 번(캠프를 뒤집어) 쳐서 자리 편향을 씻는다.
 * 두 캠프 모두 mixoracle 이고 차이는 좌석 게이트(__mightyForceLegacy) 하나뿐이다.
 *
 *   node sim_mighty_hold_mighty_ab.js [deals] [seedBase]   ← 실제 대국 수는 ×2
 *   NULL=1 node sim_mighty_hold_mighty_ab.js ...   ← 두 캠프 같은 규칙(A/A 검증)
 *
 * 이 수정이 걸리는 국면 자체가 드물어서(300라운드에 0~2회) 헤드라인 점수차는
 * 노이즈에 묻힌다. NULL=1 로 하네스 노이즈 폭을 먼저 재고, 그 폭 안이면
 * "손해 없음"으로 읽어야 한다. 낭비 자체가 사라졌는지는
 * sim_mighty_mighty_waste.js 로 따로 본다.
 */
...
for (let pass = 0; pass < rounds * 2; pass++) {
  ...
  global.__mightyForceLegacy = (pid) => (!nullTest && seatCamp[pid] === 'legacy');
...
delete global.__mightyForceLegacy;

const pct = (a, b) => (b > 0 ? `${(a / b * 100).toFixed(1)}%` : '-');
console.log(`\n마이티 아끼기 A/B — ${rounds}딜 ×2(미러) (base ${base})${nullTest ? ' · NULL(A/A) 검증' : ''}`);
