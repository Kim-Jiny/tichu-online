 ...
 * 두 캠프 모두 mixoracle 이고 차이는 좌석 게이트(__mightyOppTopDumpLegacy) 하나뿐이다.
 *
...
for (let pass = 0; pass < rounds * 2; pass++) {
  ...
  global.__mightyOppTopDumpLegacy = (pid) => (!nullTest && seatCamp[pid] === 'legacy');
...
delete global.__mightyOppTopDumpLegacy;

const pct = (a, b) => (b > 0 ? `${(a / b * 100).toFixed(1)}%` : '-');
console.log(`\n야당 최상위 보존 A/B — ${rounds}딜 ×2(미러) (base ${base})${nullTest ? ' · NULL(A/A) 검증' : ''}`);
