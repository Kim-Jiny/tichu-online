'use strict';
/**
 * A-B harness: 버릴 때 자기 무늬 최상위를 남기는 좌석(hold) vs 예전 좌석(legacy).
 * 한 테이블에서 같은 시드 딜을 두 번(캠프를 뒤집어) 쳐서 자리 편향을 씻는다.
 * 두 캠프 모두 mixoracle 이고 차이는 좌석 게이트(__mightyDeclarerJokerThreatLegacy) 하나뿐이다.
 *
...
for (let pass = 0; pass < rounds * 2; pass++) {
  ...
  global.__mightyDeclarerJokerThreatLegacy = (pid) => (!nullTest && seatCamp[pid] === 'legacy');
...
delete global.__mightyDeclarerJokerThreatLegacy;

const pct = (a, b) => (b > 0 ? `${(a / b * 100).toFixed(1)}%` : '-');
console.log(`\n주공 조커콜 위협 대비 A/B — ${rounds}딜 ×2(미러) (base ${base})${nullTest ? ' · NULL(A/A) 검증' : ''}`);
